...
const EventDetails = () => {
    ...
    useEffect(() => {
        const fetchData = () => {
            let config = {
                ...
                url: `http://3.138.92.54:3000/api/getAllEvents`,
                headers: {}
            };
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const store = await axios.get(`http://3.138.92.54:3000/api/getAllStore`);
                setPopularStore(store.data.stores);
                const response = await axios.get(`http://3.138.92.54:3000/api/events/${event}`);
                const validCoupons = await Promise.all(response.data.coupons.map(async (c) => {
                    const coupons = await axios.get(`http://3.138.92.54:3000/api/coupons/${c.store_id}/${c.coupon_id}`);
                    // console.log(coupons);
