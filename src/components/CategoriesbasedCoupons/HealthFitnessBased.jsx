...
const HealthFitnessBased = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("http://3.138.92.54:3000/api/storeDisplay/Health Fitness");
            ...
            <div className="clothing mb-5">
                <div className="flex flex-col justify-center items-center lg:flex-row lg:justify-between my-10 h-[2rem]">
                    <div className="flex flex-col gap-1 lg:gap-5">
                        <span className="font-semibold text-lg lg:text-3xl">Health & Fitness Offers</span>
                    </div>
                    <div className="hover:underline h-7 duration-300 underline cursor-pointer"
                        onClick={() => {
                            navigate("/categoriesdetails", { state: { category: "Health Fitness", category_icon: Health_Fitness } })
                        }}>
                        View All Health & Fitness Offers
                    </div>
                </div>
...
export default HealthFitnessBased;
